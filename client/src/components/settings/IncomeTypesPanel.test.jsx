// IncomeTypesPanel.jsx tests. Per the component's own header comment, this is "ExpenseTypesPanel.jsx
// ... mirrored exactly for the income side of the same feature" - so this test file mirrors
// ExpenseTypesPanel.test.jsx's structure and coverage accordingly (loading/empty/populated states,
// businessScopeFor's shop-vs-artist split, add-with-validation, and the active/inactive toggle),
// adjusted for IncomeService's own field names (incomeTypeId rather than expenseTypeId, etc.).
//
...
import IncomeTypesPanel from "./IncomeTypesPanel";
import IncomeService from "../../services/IncomeService";
import { AuthContext } from "../../context/auth";
import { ROLES } from "../../constants/auth";

// THE REAL DOCUMENTS, imported from the service - not copies. IncomeService exports
// FETCH_INCOME_TYPES/CREATE_INCOME_TYPE/UPDATE_INCOME_TYPE directly, and it's the SAME object
// reference the wrapped hook uses internally (see IncomeService.test.js's own header comment), so
// mocks below reference IncomeService.FOO rather than reconstructing a local copy of the query text.
...
function incomeType(overrides = {}) {
	return {
		...
		artistUserId: null,
		name: "Retail/Merch",
		description: "T-shirts and prints",
		active: true,
...
describe("loading", () => {
	it("shows neither the list nor the empty message while the query is in flight", () => {
		renderPanel({
			user: SHOP_ADMIN,
			mocks: [typesMock({ scope: SHOP_SCOPE, types: [incomeType()] })],
		});

		expect(screen.queryByText("Retail/Merch")).not.toBeInTheDocument();
		expect(screen.queryByText("No income categories yet.")).not.toBeInTheDocument();
	...
	it("renders each type's name, description, and Deactivate action", async () => {
		renderPanel({
			...
			mocks: [
				typesMock({
					scope: SHOP_SCOPE,
					types: [incomeType({ name: "Retail/Merch", description: "T-shirts and prints" })],
				}),
			],
		});

		expect(await screen.findByText("Retail/Merch")).toBeInTheDocument();
		expect(screen.getByText("T-shirts and prints")).toBeInTheDocument();
	...
	it("marks an inactive type with a chip and offers Reactivate instead of Deactivate", async () => {
		renderPanel({
			...
			mocks: [
				typesMock({
					scope: SHOP_SCOPE,
					types: [incomeType({ name: "Old category", active: false })],
				}),
			],
		});
	...
	it("omits the description span entirely when a type has none", async () => {
		renderPanel({
			...
			mocks: [
				typesMock({
					scope: SHOP_SCOPE,
					types: [incomeType({ name: "No description", description: "" })],
				}),
			],
		});
...
describe("scope by caller", () => {
	it("queries by shopId for a shop admin with a shop", async () => {
		renderPanel({
			user: SHOP_ADMIN,
			mocks: [typesMock({ scope: SHOP_SCOPE, types: [incomeType({ name: "Shop-scoped" })] })],
		});
	...
	it("queries by artistUserId for an independent artist", async () => {
		renderPanel({
			...
			mocks: [
				typesMock({
					scope: ARTIST_SCOPE,
					types: [incomeType({ id: "income-type-2", shopId: null, artistUserId: "artist-1", name: "Artist-scoped" })],
				}),
			],
		});
...
describe("adding a category", () => {
	it("disables Add Category until a name is typed", async () => {
		...
		await screen.findByText("No income categories yet.");
	...
	it("enables Add Category once a name is typed", async () => {
		...
		await screen.findByText("No income categories yet.");
		await user.type(screen.getByLabelText("New category name"), "Piercing");
	...
	it("creates a type scoped to the shop, trims name/description, and refetches the list", async () => {
		const user = userEvent.setup();
		const input = { shopId: "shop-1", name: "Piercing", description: "Walk-in piercing services" };
		const createMock = {
			request: { query: IncomeService.CREATE_INCOME_TYPE, variables: { input } },
			result: { data: { createIncomeType: incomeType({ id: "income-type-2", name: "Piercing", description: "Walk-in piercing services" }) } },
		};
		renderPanel({
			user: SHOP_ADMIN,
			mocks: [
				...
				createMock,
				typesMock({ scope: SHOP_SCOPE, types: [incomeType({ id: "income-type-2", name: "Piercing", description: "Walk-in piercing services" })] }),
			],
		});
		...
		await user.type(screen.getByLabelText("New category name"), "  Piercing  ");
		await user.type(screen.getByLabelText("Description (optional)"), "  Walk-in piercing services  ");
		await user.click(screen.getByRole("button", { name: "Add Category" }));

		expect(await screen.findByText("Piercing")).toBeInTheDocument();
	});

	it("creates a type with just createScopeFor's subset for an independent artist (no artistUserId field)", async () => {
		const user = userEvent.setup();
		// createScopeFor drops artistUserId entirely for an independent artist - CreateIncomeTypeInput
		// has no such field, so the input carries only name/description.
		const input = { name: "Booth rent collected", description: "" };
		const createMock = {
			request: { query: IncomeService.CREATE_INCOME_TYPE, variables: { input } },
			result: {
				data: {
		...
		renderPanel({
			...
			mocks: [
				...
				typesMock({
					scope: ARTIST_SCOPE,
					types: [incomeType({ id: "income-type-3", shopId: null, artistUserId: "artist-1", name: "Booth rent collected", description: "" })],
				}),
			],
		});

		await screen.findByText("No income categories yet.");
		await user.type(screen.getByLabelText("New category name"), "Booth rent collected");
		await user.click(screen.getByRole("button", { name: "Add Category" }));

		expect(await screen.findByText("Booth rent collected")).toBeInTheDocument();
	});
	...
	it("clears the form after a successful add", async () => {
		...
		renderPanel({
			...
			mocks: [
				typesMock({ scope: SHOP_SCOPE, types: [] }),
				createMock,
				typesMock({ scope: SHOP_SCOPE, types: [incomeType({ id: "income-type-2", name: "Piercing" })] }),
			],
		});

		await screen.findByText("No income categories yet.");
		const nameField = screen.getByLabelText("New category name");
		await user.type(nameField, "Piercing");
		await user.click(screen.getByRole("button", { name: "Add Category" }));

		await screen.findByText("Piercing");
...
describe("toggling active state", () => {
	it("deactivates an active type and refetches", async () => {
		...
		const updateMock = {
			request: {
				query: IncomeService.UPDATE_INCOME_TYPE,
				variables: { input: { incomeTypeId: "income-type-1", active: false } },
			},
			result: { data: { updateIncomeType: incomeType({ active: false }) } },
		};
		renderPanel({
			user: SHOP_ADMIN,
			mocks: [
				typesMock({ scope: SHOP_SCOPE, types: [incomeType({ active: true })] }),
				updateMock,
				typesMock({ scope: SHOP_SCOPE, types: [incomeType({ active: false })] }),
			],
		});
	...
	it("reactivates an inactive type and refetches", async () => {
		...
		const updateMock = {
			request: {
				query: IncomeService.UPDATE_INCOME_TYPE,
				variables: { input: { incomeTypeId: "income-type-1", active: true } },
			},
			result: { data: { updateIncomeType: incomeType({ active: true }) } },
		};
		renderPanel({
			user: SHOP_ADMIN,
			mocks: [
	...
	it("alerts the server's error message when the toggle fails", async () => {
		...
		const { setAlert } = renderPanel({
			user: SHOP_ADMIN,
			mocks: [typesMock({ scope: SHOP_SCOPE, types: [incomeType({ active: true })] }), failingMock],
		});
