...
**Still open:** the second run's `9 failed | 53 passed` doesn't fully resolve to the fixes above - `IBDateTimePicker.test.jsx` (2) and `IBPasswordField.test.jsx` (2) accounted for 4 of the 9 in the shown output; `IBDatePicker.test.jsx` likely accounts for another 2 given its identical structure (fixed pre-emptively, not confirmed), leaving up to 3 tests in at least one more file neither seen nor diagnosed yet. Re-run `npm test` in `client/` after pulling these fixes and paste the full, non-truncated output - don't assume everything's covered until a run actually comes back clean.
