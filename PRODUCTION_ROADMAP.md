...
**Not yet confirmed clean.** Four rounds of "should be fixed now" so far, each surfacing something the previous fix didn't anticipate - this is exactly why "confirm with a real run" matters more than "looks right on paper." Run `npm test` in `client/` once more before treating this as closed.
