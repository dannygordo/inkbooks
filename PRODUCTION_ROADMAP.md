...
**Run for real again after the twelve new `IB*` component test files were added (August 3, 2026): 12 failed / 50 passed initially, all three root causes now fixed.** Same lesson as August 2 - this sandbox can't execute Vitest at all (`Cannot find module @rollup/rollup-linux-x64-gnu`, blocked npm install), so these test files had only ever been syntax-checked, never run, until the user ran them for real. Two distinct, real bugs:
- **`IBEmailField.jsx`/`IBPasswordField.jsx` threw `ReferenceError: React is not defined`** under Vitest - the exact same automatic-JSX-runtime-vs-Vitest-transform gap noted above for `Login.jsx`/`Register.jsx` (and already worked around in `IBDatePicker.jsx`/`IBDateTimePicker.jsx`), just not yet hit for these two, since nothing had ever actually mounted them in a test before. Fixed the same way: explicit `import React from 'react'` added to both.
- **`IBDateTimePicker.test.jsx` failed both its tests with `TestingLibraryElementError: Found multiple elements with the text of: Appointment Date/Time`** (from `getByLabelText`) - not a component bug. MUI X's Date/Time pickers (v7+) render an "accessible field" DOM structure: a visually-hidden, `aria-hidden`, form-submittable `<input>` sitting alongside a `role="group"` container of editable sections (day/month/year/etc.), and both get the same accessible name from the `label` prop - which is exactly what makes a plain `getByLabelText()` ambiguous. Confirmed against MUI X's own "Testing caveats" docs, which recommend targeting that hidden input directly rather than the group. Fixed by querying `screen.getByRole("textbox", { hidden: true, name: label })` instead (Testing Library's equivalent of the `includeHidden: true` option MUI's own docs use for Playwright) in both `IBDateTimePicker.test.jsx` and `IBDatePicker.test.jsx` (same underlying structure, same fragile query, pre-emptively fixed there too even though only the DateTime one was confirmed failing in the pasted output).
- The reported run only showed 3 of the 5 failing test files' detail (output appears to have been truncated mid-scroll) - the other 2 were not yet identified when these fixes were made. Re-run `npm test` in `client/` after pulling these fixes to confirm the full suite is green; if anything is still red, it needs its own diagnosis rather than being assumed covered by the above.
