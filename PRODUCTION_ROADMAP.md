...
- **Resolver-level auth wrapper.** Write a `withAuth(minRole, resolverFn)` higher-order function, or use a GraphQL schema directive (`@auth(requires: SHOP_ADMIN)`), and apply it uniformly. New resolvers inherit the protection by default instead of needing someone to remember to add it.
- **Input validation library.** Replace the hand-rolled `validators.js` with `zod`. It gives you schema-based validation with less code and catches the "silently ignored extra field" class of bug (like `tagColor` never making it into `validateRegisterInput`'s signature).
- **Ownership-scoped queries.** Right now most `getX` queries return everything in the collection. A Client's `getClient` should only resolve their own record; a Shop Admin's `getArtists` should scope to their shop, not the whole database. This is as much a data-model decision as a code fix — worth deciding shop-level tenancy rules explicitly before implementing.
- **Firebase Auth, done properly.** Replace the shared static account (`firebase@inkbooks.net`) with real per-user Firebase Auth: your Express backend mints a custom Firebase token via the Admin SDK at login time (using the user's own Mongo `_id` as the Firebase UID), and Firebase Storage security rules get written per-user instead of "one account can do everything." This also gets the shared password out of the client bundle entirely.
- **Consolidate the two backend listeners.** Right now Apollo Server runs standalone on port 5500 and `socket.io` runs separately on port 4000. Merging them into one Express app (Apollo as Express middleware + socket.io attached to the same HTTP server) simplifies CORS, deployment, and gives you one process to monitor instead of two.
...
|---|---|---|---|
| `apollo-server` (standalone) | 3.6.2 | — (package superseded) | `apollo-server` is EOL; the replacement is `@apollo/server` (currently 5.5.1), which requires attaching to an Express app rather than running standalone — this upgrade and the "consolidate listeners" work in Phase 2 should happen together. |
