...
**Not yet confirmed clean.** These fixes address every failing test shown in the third run's (now complete, non-truncated) output - but as with the prior two rounds, this hasn't been re-run for real yet. Run `npm test` in `client/` once more before treating this as closed.
